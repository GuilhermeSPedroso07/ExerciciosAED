...
int fourSumCount( int *nums1, int nums1Size, int *nums2, int nums2Size,
	...
	int nums34Size = nums3Size * nums4Size;
	int *nums12 = ( int * ) malloc ( sizeof ( int ) * nums12Size );
	int *nums34 = ( int * ) malloc ( sizeof ( int ) * nums34Size );
	int soma;
	int ab = 0, cd = nums34Size - 1;
	int abCount = 0, cdCount = 0; 

	GerarSomasDePares ( nums12, nums1, nums2, nums1Size );
	GerarSomasDePares ( nums34, nums3, nums4, nums3Size );
	qsort ( nums12, nums12Size, sizeof ( int ), CompararInt );
	qsort ( nums34, nums34Size, sizeof ( int ), CompararInt );

	while ( ab < nums12Size && cd >= 0 ) {
		soma = nums12[ab] + nums34[cd];
		if ( soma > 0 ) {
			cd--;
			while ( cd >= 0 && nums34[cd + 1] == nums34[cd] ) {
				cd--;
			}
		} else if ( soma < 0 ) {
			ab++;
			while ( ab < nums12Size &&  nums12[ab - 1] == nums12[ab] ) {
				ab++;
			}
		} else {
			ab++;
			...
			abCount++;
			cdCount++;
			while ( ab < nums12Size &&  nums12[ab] == nums12[ab - 1] ) {
				abCount++;
				ab++;
			}
			while ( cd >= 0 && nums34[cd] == nums34[cd + 1] ) {
				cdCount++;
				cd--;
			}
