...
qsort ( nums1, nums1Size, sizeof ( int ), CompararInt );
qsort ( nums2, nums1Size, sizeof ( int ), CompararInt );
qsort ( nums3, nums1Size, sizeof ( int ), CompararInt );
qsort ( nums4, nums1Size, sizeof ( int ), CompararInt );

int a, b, c, d, result;
result = 0;

long int soma = nums1[0] + nums2[0] + nums3[0] + nums4[0];
if ( soma > 0 ) {
	return 0;
}
for ( int i = 0; i < nums1Size && soma <= 0; i++ ) {
	for ( int j = 0; j < nums1Size && soma <= 0; j++ ) {
		for ( int k = 0; k < nums1Size && soma <= 0; k++ ) {
			for ( int l = 0; l < nums1Size && soma <= 0; l++ ) {
				soma = nums1[i] + nums2[j] + nums3[k] + nums4[l];
				if ( soma == 0 ) {
					result++;
				}
				soma = 0;
