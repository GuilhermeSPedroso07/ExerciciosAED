...
====================
*/
no_t *Inserir_r( no_t *raiz, int valor, int index ) {
	no_t *	n;
	...
	if ( valor < raiz->valor ) {
		raiz->esq = Inserir_r( raiz->esq, valor, index );
	} else {
		raiz->dir = Inserir_r( raiz->dir, valor, index );
	}
...
int ContarMaiores( const no_t *raiz, int valor ) {
	if ( !raiz ) {
		return 0;
	}
	if ( valor < raiz->valor ) {
		return 1 + Tamanho( raiz->dir ) + ContarMaiores( raiz->esq, valor );
	}
...
isIdealPermutation
	...
	Retorna true se toda inversao global tambem for uma inversao
	local (isto e, nao existe nums[k] > nums[j] com j - k >= 2).
...
bool isIdealPermutation( int *nums, int numsSize ) {
	...
	for ( int i = 2; i < numsSize; i++ ) {
		root = Inserir_r( root, nums[i - 2], i - 2 );
		if ( ContarMaiores( root, nums[i] ) > 0 ) {
			resultado = false;
			break;
		}
	}
...
int main( void ) {
	...
	int		t1[]  = { 2, 1, 0 };				/* exemplo 1 do enunciado             */
