...
void inorder ( struct TreeNode* root, int *output, int *amount ) {
    if ( root == NULL ) {
        return;
    }

    inorder ( root->left, output, amount    );
    output[*amount] = root->val;
    ( *amount )++;
    inorder ( root->right, output, amount );
}
...
int main() {
    ...
    int size;
    int *result = inorderTraversal(n4, &size);
